...
namespace SalarSoft.ASProxy.Update
{
	...
	public class Updater
	{
		...
		private static void UpdateReminder_Callback(object sender, ElapsedEventArgs e)
		{
			...
			{
				ASProxyExceptions.LogException(ex, "Error in Updater.UpdateReminder_Callback");
			}
		}
		...
		public Updater()
		{
			_AutoUpdateInfoUrl = ASProxyConfig.ASProxyAutoUpdateInfoUrl;
			_CurrentVersion = GlobalConsts.ASProxyVersion;
		}
		...
		{
			byte[] bytes = GetDataBytes(ASProxyConfig.ASProxyAutoUpdateInfoUrl);
		...
		private string GetUpdateHoldingPlace()
		{
			...
			appPath = Path.Combine(appPath, FilesConsts.Dir_Bin + sep + FilesConsts.Dir_UpdateSources + sep);
		...
		private void GetDataFile(string url,string destFilename)
		{
			using (WebClient client = new WebClient())
			{
				...
				{
					client.Proxy = ASProxyConfig.GenerateWebProxy();
				}
