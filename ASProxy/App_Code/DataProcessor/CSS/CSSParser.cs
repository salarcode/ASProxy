...
namespace SalarSoft.ASProxy
{
	...
	public class CSSParser
	{
		...
		public static TextRange FindImportRuleUrlPosition(ref string cssCodes, int startindex, bool onlyWithoutUrlOption, bool captureExactlyInUrl)
		{
			...
			// Find first position
			valueStart = Performance.IndexOfIgnoreCase(ref cssCodes, strCSSImportRule, startindex);
			if (valueStart == -1)
				return result;
			...
			valueEnd = Performance.IndexOfMatchCase(ref cssCodes, ";", valueStart);

			if (valueEnd == -1)
				return result;
			else
			{
				int urlPos = Performance.IndexOfIgnoreCase(ref cssCodes, strCSSUrlValue, valueStart);
				if (urlPos != -1 && urlPos < valueEnd)
				{
					...
					if (captureExactlyInUrl)
					{
						...
						urlPos = Performance.IndexOfMatchCase(ref cssCodes, ")", valueStart);
						if (urlPos != -1 && urlPos < valueEnd)
						{
							valueEnd = urlPos;
						}
		...
		public static TextRange FindCSSClassStyleUrlValuePosition(ref string pageHtml, int startindex)
		{
			...
			// Find first position
			valueStart = Performance.IndexOfIgnoreCase(ref pageHtml, strCSSUrlValue, startindex);
			if (valueStart == -1)
				return result;

			valueStart += strCSSUrlValue.Length;
			valueEnd = Performance.IndexOfMatchCase(ref pageHtml, ")", valueStart);

			if (valueEnd == -1)
				return result;

			if (valueEnd > Performance.IndexOfMatchCase(ref pageHtml, ";", valueStart))
				return result;
