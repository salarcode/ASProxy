...
namespace SalarSoft.ASProxy
{
	...
	public class CSSReplacer
	{
		...
		/// </summary>
		public static void ReplaceStyleTagStyleUrl(ref string htmlCode, string currentUrlWithoutParameters, string importRuleUrl, string othersNewUrl, string replacmentBasePath, string siteAddress, bool encodeUrl)
		{
			...
			do
			{
				...
				position.Start = Performance.IndexOfIgnoreCase(ref htmlCode, styleStart, index);
				if (position.Start == -1)
					break;
				...
				// Locate end tag
				position.End = Performance.IndexOfIgnoreCase(ref htmlCode, styleEnd, position.Start);
				if (position.End == -1)
					break;
		...
		public static void ReplaceCSSClassStyleUrl(ref string htmlCode, string currentUrlWithoutParameters, string newUrl, string replacmentBasePath, string siteAddress, bool encodeUrl, bool forImportRule)
		{
			...
			do
			{
				...
				if (position.Start != -1 && position.End != -1)
				{
					...
					if (Performance.IndexOfMatchCase(ref oldValue, '#') != -1)
				...
				else
				{
					...
					index = Performance.IndexOfMatchCase(ref htmlCode, ' ', index);
				}
