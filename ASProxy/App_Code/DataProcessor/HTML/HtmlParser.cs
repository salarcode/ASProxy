...
namespace SalarSoft.ASProxy
{
	...
	public class HtmlParser
	{
		...
		public static void RemoveTag(ref string pageHtml, string tagName, bool removeTag)
		{
			...
			if (removeTag)
			{
				do
				{
					tagStartPos = Performance.IndexOfIgnoreCase(ref pageHtml, tagStart, tagStartPos);
					if (tagStartPos == -1)
						return;
					tagStartEndPos = Performance.IndexOfMatchCase(ref pageHtml, tagStartEnd, tagStartPos);

					if (tagStartEndPos == -1)
						return;
					tagStartEndPos += tagStartEnd.Length;

					tagEndPos = Performance.IndexOfMatchCase(ref pageHtml, tagEnd, tagStartEndPos);
		...
		public static TextRange GetTagAttributeValuePos(ref string pageHtml, string tagName, string attributeName, int start)
		{
            ...
            tagStart = Performance.IndexOfIgnoreCase(ref pageHtml, tagName, start);
			if (tagStart == -1)
				return result;

			// Locate end of tag
			tagEnd = Performance.IndexOfMatchCase(ref pageHtml,">", tagStart);
            ...
            attrStart = Performance.IndexOfIgnoreCase(ref pageHtml, attributeName, tagStart);
            ...
            valueStart = Performance.IndexOfIgnoreCase(ref pageHtml, '=', attrStart);
        ...
        public static string GetTagContent(ref string pageHtml, string justTagName)
        {
            ...
            start = Performance.IndexOfIgnoreCase(ref pageHtml, startTag);
            if (start == -1)
                return "";
            start = Performance.IndexOfMatchCase(ref pageHtml, '>', start);
            if (start == -1)
                return "";
            start++;

            end = Performance.IndexOfIgnoreCase(ref pageHtml, endTag, start);
            if (end == -1)
                return "";
            end = Performance.LastIndexOfMatchCase(ref pageHtml, '<', end);
            if (end == -1 || start > end)
                return "";
