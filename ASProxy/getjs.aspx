...
try
{
	if (!Page.IsPostBack)
	{
		...
		{
			engine = new ASProxyEngine(ProcessTypeForData.JavaScript,true);
			engine.RequestInfo.ContentType = MimeContentType.text_javascript;
			...
			if (engine.LastStatus== LastActivityStatus.Error)
			{
				...
				Response.Write("/* Status Error: " + engine.LastErrorMessage + " */");
				Response.End();
			}
		}
		else
